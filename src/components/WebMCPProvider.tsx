...
function makeTools(whatsappBase: string, urlWithText: (text: string) => string): ToolDef[] {
  return [
    {
      ...
      execute: () => {
        const url = urlWithText("Hi Lotus365, I want a new ID.");
    ...
    },
    {
      ...
      annotations: { readOnlyHint: false, untrustedContentHint: false },
      execute: () => {
        const url = urlWithText("Hi, I want to claim the ₹25,000 welcome bonus.");
        window.open(url, "_blank");
    ...
    },
    {
      ...
      annotations: { readOnlyHint: false, untrustedContentHint: false },
      execute: (input) => {
        const msg = typeof input?.message === "string" ? input.message : "Hi Lotus365 support";
        window.open(urlWithText(msg), "_blank");
        return { ok: true };
      },
    },
    {
...
export function WebMCPProvider() {
  ...
  useEffect(() => {
    const tools = makeTools(url, urlWithText);
