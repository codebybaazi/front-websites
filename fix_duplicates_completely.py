...
def get_unique_content(slug, title, q_idx, post_idx):
    ...
    q_stem_idx = (post_idx + q_idx) % 20
    a_stem_idx = (post_idx + q_idx + 5) % 20
...
for i, slug in enumerate(slugs):
    # Find the data block for this specific slug more accurately
    pattern = re.compile(f'"{slug}":\\s*\\[(.*?)\n\\s*\\]', re.DOTALL)
    ...
    new_faqs = [get_unique_content(slug, title, j, i*4) for j in range(4)]
